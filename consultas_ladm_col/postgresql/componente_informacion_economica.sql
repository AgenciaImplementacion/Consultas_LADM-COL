WITH
 ...
 info_calificacion_convencional AS (
	SELECT avaluounidadconstruccion.aucons,
		...
		) FILTER(WHERE calificacion_convencional.t_id IS NOT NULL) AS calificacion_convencional
	...
	GROUP BY avaluounidadconstruccion.aucons
 ),
 info_calificacion_no_convencional AS (
	SELECT avaluounidadconstruccion.aucons,
		...
		) FILTER(WHERE calificacion_no_convencional.t_id IS NOT NULL) AS calificacion_no_convencional
	 ...
	 SELECT unidadconstruccion.construccion,
			json_agg(json_build_object('id', unidadconstruccion.t_id,
							  'attributes', json_build_object('Número de pisos', unidadconstruccion.numero_pisos
															  ...
															  , 'calificacion_convencional', COALESCE(info_calificacion_convencional.calificacion_convencional, '[]')
															  , 'calificacion_no_convencional', COALESCE(info_calificacion_no_convencional.calificacion_no_convencional, '[]')
	...
	SELECT terreno.t_id,
		json_agg(
				json_build_object('id', zona_homogenea_geoeconomica.t_id,
									   'attributes', json_build_object('Porcentaje', (st_area(st_intersection(terreno.poligono_creado, zona_homogenea_geoeconomica.geometria))/ st_area(terreno.poligono_creado)) * 100,
	...
	SELECT terreno.t_id,
		json_agg(
				json_build_object('id', zona_homogenea_fisica.t_id,
									   'attributes', json_build_object('Porcentaje', (st_area(st_intersection(terreno.poligono_creado, zona_homogenea_fisica.geometria))/ st_area(terreno.poligono_creado)) * 100,
