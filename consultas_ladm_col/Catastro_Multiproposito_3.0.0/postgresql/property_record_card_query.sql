WITH
 ...
 fcm_contacto_visita AS (
	...
	FROM operacion.fcm_contacto_visita WHERE fcm_contacto_visita.fcm_formulario IN (SELECT * FROM predio_formulario_unico)
	 ...
	 SELECT col_uebaunit.ue_op_terreno,
			json_agg(json_build_object('id', op_predio.t_id,
							  'attributes', json_build_object('Nombre', op_predio.nombre
															  ...
															  , 'Formalidad', (SELECT dispname FROM operacion.fcm_formalidadtipo WHERE t_id = fcm_formulario_unico_cm.formalidad)
															  ...
															  , 'Clase suelo', (SELECT dispname FROM operacion.fcm_clasesuelotipo WHERE t_id = fcm_formulario_unico_cm.clase_suelo)
															  , 'Categoría suelo', (SELECT dispname FROM operacion.fcm_categoriasuelotipo WHERE t_id = fcm_formulario_unico_cm.categoria_suelo)
