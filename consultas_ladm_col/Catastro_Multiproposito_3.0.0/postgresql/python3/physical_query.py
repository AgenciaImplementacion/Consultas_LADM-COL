def get_igac_physical_query(schema, plot_t_id, parcel_fmi, parcel_number, previous_parcel_number, valuation_model):
    ...
    if valuation_model:
        query += """
            															  ...
            															  'Tipo', (SELECT dispname FROM {schema}.av_unidadconstrucciontipo WHERE t_id = av_unidad_construccion.tipo_unidad_construccion),
        """
    else:
        query += """
            															  'Puntuación', NULL,
    ...
    query += """
            ...
            info_puntolevantamiento AS (
                ...
                FROM
                (
                    SELECT op_puntolevantamiento.t_id AS t_id_puntolevantamiento, op_puntolevantamiento.localizacion_original, op_construccion.t_id AS t_id_construccion  FROM {schema}.op_construccion, {schema}.op_puntolevantamiento
